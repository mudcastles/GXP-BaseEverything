...
class SubjectOptionLayout : ViewGroup {
     ...
     * checkbox的点击事件
     */
    private var mOnCheckedChangeListener =
    ...
    private fun initValue(attrs: AttributeSet?) {
        ...
        if (ta != null) {
            mDrawableSize =
                ta.getDimension(
                    R.styleable.SubjectOptionLayout_drawableSize,
                    DimenUtil.dip2px(context, 14f).toFloat()
                )
                    .toInt()
    ...
    fun addOption(itemContent: String, optionStatus: OptionStatus) {
        ...
        val mItemView =
            LayoutInflater.from(context).inflate(R.layout.item_subject_option, this, false)
