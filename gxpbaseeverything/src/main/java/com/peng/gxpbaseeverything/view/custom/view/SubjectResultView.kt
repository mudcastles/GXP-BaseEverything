...
import com.peng.gxpbaseeverything.view.custom.model.Subject
...
class SubjectResultView : View {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(
            measuredWidth,
            paddingTop + paddingBottom + 2 * mCircleRadius + mGapY * (Math.ceil(mSubjects.size.toDouble() / mSubjectNumPeerRow)
                .toInt() - 1)
        )
    }
    ...
    private fun calValues() {
        ...
        mGapX =
            (measuredWidth - mCircleRadius * 2 - paddingStart - paddingEnd) / (mSubjectNumPeerRow - 1)
